...
export function receiveCommandRemove(userMessageContent, messageStack) {
  try {
    ...
    console.log(
      `User: ${messageStack.member.user.tag} is removing ${channelName} from ${platform}`
    );

    const removed = removeChannelFromJson(channelName, platform);
    if (removed) {
      messageStack.channel.send(
        `Channel ${channelName} removed from watch list`
      );
    } else {
      messageStack.channel.send(
        `Channel ${channelName} is not in the watch list`
      );
    }
  } catch (error) {
    console.error(error);
    messageStack.channel.send(
      "Error occurred while removing the channel from watch list."
    );
  }
}
...
function removeChannelFromJson(channelName, platform) {
  ...
  try {
    ...
    fs.writeFileSync(
      filePath,
      JSON.stringify({ [platform]: filteredChannels })
    );
    console.log(`Removed ${channelName} from ${platform}.json`);
    return true;
  } catch (err) {
    console.error(
      `Error removing channel from ${platform}.json: ${err.message}`
    );
    return false;
  }
}
