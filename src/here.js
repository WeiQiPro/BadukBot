...
export function receiveCommandHere (userMessageContent, messageStack) {
  console.log(
    `User: ${messageStack.member.user.tag} is adding ${messageStack.channel.name} to ${messageStack.guild.name}`
  )
...
function validateChannelType (userMessageContent, messageStack) {
  ...
  if (!['notifications', 'tsumego', 'battles', 'sgf'].includes(channelType)) {
    messageStack.channel.send(
      'Please specify notifications, tsumego, battles, or sgf.'
    )
    return null
  }
  return channelType
  ...
  try {
    ...
    if (guildIndex >= 0) {
      if (guilds[guildIndex].channels[channels.type]) {
        ...
        await messageStack.channel.send(
        ...
        const collected = await messageStack.channel.awaitMessages({
          filter,
          max: 1,
          time: 30000,
          errors: ['time']
    ...
    messageStack.channel.send(
      `Successfully added channel ${channels.name} to ${channels.type} in ${guild.name}`
    )
    console.log(
      `Successfully added channel ${channels.name} to ${channels.type} in ${guild.name}`
    )
